...
# Reference from https://github.com/qiuqiangkong/audioset_tagging_cnn/blob/master/pytorch/models.py
...
# from .feature_fusion import iAFF, AFF, DAF  # Missing module
...
def create_pann_model(audio_cfg):
    try:
        ...
        model = ModelProto(
            ...
            fmin = audio_cfg.fmin,
            fmax = audio_cfg.fmax,
            classes_num = audio_cfg.classes_num
        )
        return model
    except:
...
def create_transfer_pann_model(audio_cfg):
    try:
        ModelProto = eval(audio_cfg.model_name)
        model = ModelProto(
            sample_rate = audio_cfg.sample_rate,
            ...
            fmax = audio_cfg.fmax,
            classes_num = audio_cfg.classes_num,
            freeze_base = audio_cfg.freeze_base
        )
        return model
    except:
        raise RuntimeError(f'Import Model for {audio_cfg.model_name} not found, or the audio cfg parameters are not enough.')