...
Enhanced Features:
- Command-line interface with configurable experiments and parameters
- Robust error handling that continues evaluation despite individual failures
- Comprehensive logging with progress tracking and status reporting
- Configuration validation with detailed error messages
- Dataset and checkpoint validation before expensive operations
- Modular architecture for better maintainability and testing
- Statistical analysis with configurable significance testing
"""
...
from music_anomalizer.utils import (
    load_pickle, cleanup, 
    plot_score_distributions, box_plot_anomaly_scores
)
...
def validate_dataset(dataset_path: str, dataset_name: str) -> Optional[Any]:
    ...
    try:
        ...
        if data is None or len(data) == 0:
            logger.error(f"❌ Dataset loaded as None or empty: {dataset_name}")
            return None
        
        logger.debug(f"✅ Dataset '{dataset_name}' loaded: {len(data)} samples ({file_size / (1024*1024):.1f} MB)")
        return data
...
def evaluate_baseline_models(
    ...
    """
    logger = logging.getLogger(__name__)
    logger.info("🏗️  Starting baseline model evaluation")
            ...
            try:
                ...
                if enable_visualization:
                    try:
                        ...
                        logger.warning(f"⚠️  Visualization failed for {base_name}: {e}")
            ...
            for key, value in results.items():
                try:
                    if "train" in key:
                        ...
                        logger.debug(f"⚠️  Unrecognized key pattern: {key} in {dataset_name} ({source_name})")
                        
                except Exception as e:
                    logger.warning(f"⚠️  Error processing key {key} for {dataset_name}: {e}")
...
def create_visualizations(
    ...
    if not train_results or not valid_results or not thresholds:
        logger.warning("⚠️  Insufficient results for visualization")
        return
        ...
        for dataset_name in dataset_names:
            try:
                ...
                if not available_models:
                    logger.warning(f"⚠️  No complete model results found for {dataset_name}")
                    continue
...
def perform_statistical_analysis(
    ...
    if not train_results or not valid_results:
        logger.warning("⚠️  Insufficient results for statistical analysis")
        return
...
def display_benchmark_summary(
    ...
    logger.info(f"🏗️  Baseline Models:")
...
def main(
    ...
    try:
        ...
        if train_results and valid_results and thresholds:
            ...
            logger.warning("⚠️  Skipping visualizations due to insufficient results")
        
        # Perform statistical analysis
        if train_results and valid_results:
            perform_statistical_analysis(train_results, valid_results, target_dataset)
        else:
            logger.warning("⚠️  Skipping statistical analysis due to insufficient results")
