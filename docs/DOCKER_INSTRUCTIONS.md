...
The development Docker setup (`Dockerfile.dev`) provides several enhancements over the production setup:

- **Live Code Updates**: All local changes are immediately reflected in running containers
- **JupyterLab**: Enhanced notebook interface instead of classic Jupyter Notebook
- **Auto-Reload**: Streamlit automatically reloads when code changes are detected  
- **Cached Dependencies**: Pip cache is preserved across container rebuilds for faster startup
- **Development Shell**: Interactive bash shell for debugging and exploration
...
- **Live Updates**: The development setup mounts your entire project directory, so all code changes are immediately reflected
- **GPU Support**: Both setups require NVIDIA Docker runtime for GPU acceleration
- **Package Installation**: The package is installed in development mode, so local changes are immediately available
- **Configuration**: Configuration files are available in the `configs/` directory
- **Caching**: Development setup includes pip caching for faster container rebuilds
